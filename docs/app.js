...
shareConsent.addEventListener("change", () => {
  ...
  setShareStatus(shareConsent.checked ? getShareReadyMessage() : "Opt in to enable sharing.", "neutral");
});
...
shareForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (supabaseEnabled) {
      await window.alignedFoundationSupabase.insertSharedResult(entry);
      setShareStatus("Shared successfully. The public dashboard can now include this anonymized result set.", "success");
      return;
    }

    const savedEntries = readStoredSharedResults();
    savedEntries.push(entry);
    window.localStorage.setItem(sharedResultsStorageKey, JSON.stringify(savedEntries));
    setShareStatus("Supabase is not configured yet, so this result was saved in this browser only.", "success");
  } catch (error) {
    setShareStatus(supabaseEnabled ? "The share request did not complete. Check the Supabase configuration and RLS policies, then try again." : "This browser blocked local storage. Check privacy settings and try again.", "error");
  }
});
...
function getShareReadyMessage() {
  return supabaseEnabled
    ? "Complete the optional fields and submit to share this result set publicly through Supabase."
    : "Complete the optional fields and submit to save this result set in this browser until Supabase is configured.";
}